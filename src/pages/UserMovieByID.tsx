        ...
        <div className='flex flex-col px-4 sm:px-6'>
            ...
            <div className='flex justify-center items-center flex-col mt-20'>
                ...
                <div className='w-245 mt-5 flex flex-col mb-8'>
                    <h2 className='text-3xl font-medium'>Comments ({comments?.length})</h2>
                    <div className='flex justify-center items-center gap-2 mt-5'>
                        ...
                        />
                        <button onClick={addNewComments} className='bg-black flex justify-center items-center h-10 rounded-sm cursor-pointer hover:opacity-80'><img className='h-6 px-4' src="https://img.icons8.com/?size=100&id=bmqaSU1DN4QP&format=png&color=ffffff" alt="" /></button>
                    </div>

                    {/* load all comments */}
                    <div className='flex flex-col mt-7 gap-5'>
                        {comments && comments.map((comment) => (
                            <div key={comment.id} className='border-b border-b-neutral-200 px-4 py-3 rounded-md'>
                                <h2 className='font-medium text-lg'>{comment.name}</h2>
                                <p className='text-sm text-gray-600 mt-1'>{comment.content}</p> 
                                <p className='text-sm mt-2'>{comment?.createdAt?.toString().substring(0,10)}</p>
                            </div>
                        ))}
                    </div>
                </div>
